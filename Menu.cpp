...
int bestellung(Storage &Lager,std::vector<Supplier> &sup, Firm firm){
	...
	do {
		...
		for (auto i : sup) {
			x_++;
			a = i.getAlloy();
			std::cout<<x_<<". "<<i.getName()<<" Legierung: "<<a.getName()<<" Kupfer: "<<a.getCopper()<<"  Zink:"<<a.getZinc()<<" Zinn:"<<a.getTin()<<std::endl;

		}
		std::cout << "0. beenden" << std::endl;
		std::cin >> y_in;
		auswahl = functions::eingabe(sup.size() + 1, y_in);
		if (auswahl == -1) {
			std::cout << "Ungueltige Eingabe, bitte neu eingeben: " << std::endl;
		}
	} while (auswahl == -1);
	...
	do {
			...
			if(pos != -1)
				menge = lagerKopie.at(Lager.getAlloyPosByType(a)).getAmount();
	...
	//HTML
	std::cout << "Moechten sie ein Bestellprotokol? " << std::endl << "1. Ja ";
	Alloy kopieA = a;
	kopieA.setAmount(stof(x_in)+a.getAmount());
	sup.at(auswahl-1).setOrder(Order(stof(x_in),firm));
	Lager.editAlloyByType(kopieA, a);
	return 0;
}
...
void produktion(Storage &Lager,std::vector<Supplier> &sup, Firm firm) {
	...
	do {
		...
		do {
			...
			do {
				std::cin >> y_in;
				if (functions::eingabe(alloyVec.size() + 1, y_in) == -1) {
					std::cout << "Ungueltige Eingabe, bitte neu eingeben: " << std::endl;
				}

			} while (functions::eingabe(alloyVec.size() + 1, y_in) == -1);
	...
	unsigned int x2 = 0;
	for(auto i: Auswahl){
		bool a2 = (b2.at(Lager.getAlloyPosByType(i)).getAmount() - mengen.at(x2)) >=0;
		if(!a2){
			std::cout<<b2.at(Lager.getAlloyPosByType(i)).getName()<<" "<<"Hat nicht genug im Lager"<<std::endl;
